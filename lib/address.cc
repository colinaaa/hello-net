...
class gai_error_category : public std::error_category {
  public:
    ...
    std::string message(const int return_value) const noexcept override { return gai_strerror(return_value); }
};
...
Address::Address(const std::string &node, const std::string &service, const addrinfo &hints) : _size() {
    ...
    const int gai_ret =
        SystemCall("Getaddrinfo", getaddrinfo(node.c_str(), service.c_str(), &hints, &resolved_address));
    if (gai_ret != 0) {
    ...
    std::unique_ptr<addrinfo, decltype(addrinfo_deleter)> wrapped_address(resolved_address,
...
std::pair<std::string, uint16_t> Address::ip_port() const {
    ...
    return {ip.data(), std::stoi(port.data())};
}

std::string Address::to_string() const {
    const auto ip_and_port = ip_port();
    return ip_and_port.first + ":" + std::to_string(ip_and_port.second);
}
